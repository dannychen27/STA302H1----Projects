...
To derive the terms for the initial model, correlations of pairs of covariates 
as well as between covariates and the response variable from the correlation matrix with high correlation served as a heuristic for determining potentially significant model terms.

The top row of the pairs scatterplots for Quiz 1 - 3 scores, Weeks 1 - 4 COVID-19 times, and Weeks 1 - 4 STA302H1 study times were analyzed separately against Quiz 4 score to hypothesize a relationship between one's Quiz 4 score and each predictor variable.

Simpler (eg., linear, quadratic) relationships were prioritized over more complex relationships (e.g., 3rd order model (or higher), logarithmic, square root) for the sake of straightforward analysis and intuitive interpretation (except for perhaps the interaction terms).

I fitted the initial model using \texttt{quiz4} as the response variable; and \texttt{quiz1}, \texttt{quiz2}, \texttt{quiz3}, \texttt{covid1}, \texttt{covid2}, \texttt{covid3}, \texttt{covid4} (including 3 quadratic terms, since \texttt{covid1}, \texttt{covid2}, and \texttt{covid4} look more quadratic), 8 interaction terms (to see how consecutive COVID and study weeks change over 4 weeks), and \texttt{country} as predictor variables.
