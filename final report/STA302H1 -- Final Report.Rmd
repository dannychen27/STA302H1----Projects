...
Obtaining an independent dataset is infeasible, and the existing dataset has sufficiently many data points ($n = 143$ after removing all NAs). Therefore, it is possible to use 50% of the dataset to develop a regression model and the remaining 50% of the dataset to validate the trained model.

The mean of the residuals of the test model was $5.026 \times 10 ^ {-17}$, which is approximately 0. See page 32 in the appendix for the corresponding R output.
...
For our $t$-test, the mean of the residuals was hypothesized to be identically equal to 0. More formally,

- $H_0: \mu_{residuals} = 0$
- $H_1: \mu_{residuals} \neq 0$

The $t$-test results showed that the $p$-value is 1, which means $H_0$ fails to be rejected, meaning that $\mu_{residuals} = 0$ holds. Since 0 falls inside the 95% confidence interval for the mean of residuals of $(-0.357787, 0.357787)$, the $H_0$ fails to be rejected. Therefore, $\mu_{residuals} = 0$. See page 32 for $t$-test output.
