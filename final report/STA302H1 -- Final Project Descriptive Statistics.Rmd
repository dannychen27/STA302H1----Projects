...
```{r}
stepAIC(second_model, direction = "both")$anova
```
...
```{r}
display_residual_plot(remaining_data_no_NAs, additive_model2, quiz3)
```

```{r}
display_residual_plot(remaining_data_no_NAs, additive_model2, study1 * study2)
```

```{r}
display_residual_plot(remaining_data_no_NAs, additive_model2, study2 * study3)
```

```{r}
display_residual_plot(remaining_data_no_NAs, additive_model2, study3 * study4)
```
...
```{r}
hist(lm(additive_model2)$residuals)
mean(lm(additive_model2)$residuals)
median(lm(additive_model2)$residuals)
```

## Try Predicting on the Fitted Values

```{r}
predicted_values = predict(additive_model2)
...
```{r}
predicted_values = predict(additive_model2, testing_data_no_NAs)
predicted_values
```
