...
cleaned_sta302_performance_data <- sta302_performance_data %>%
    ...
    # Identify rows with at least 3 missing quiz marks.
...
```{r}
rows_with_at_least_one_NA = function(data) {
...
rows_with_some_NAs = cleaned_sta302_performance_data[
  rows_with_at_least_one_NA(cleaned_sta302_performance_data),
]
```

```{r}
print(as_tibble(data.frame(
  num_row_NAs = nrow(rows_with_some_NAs)
)))
```

Let's identify which rows only have 1 NA.

```{r}
which(rows_with_at_least_one_NA(cleaned_sta302_performance_data))
```
...
```{r}
num_NAs_in_column = function(predictor_variable) {
  sum(is.na(predictor_variable))
}
```

```{r}
row_nums_of_NA = function(data, predictor_variable) {
...
print(as_tibble(data.frame(
  week1_covid = num_NAs_in_column(perform_data$COVID.hours..W1.),
  week2_covid = num_NAs_in_column(perform_data$COVID.hours..W2.),
  week3_covid = num_NAs_in_column(perform_data$COVID.hours..W3.),
  week4_covid = num_NAs_in_column(perform_data$COVID.hours..W4.)
)))
```

```{R}
print(as_tibble(data.frame(
  week1_sta302 = num_NAs_in_column(perform_data$STA302.hours..W1.),
  week2_sta302 = num_NAs_in_column(perform_data$STA302.hours..W2.),
  week3_sta302 = num_NAs_in_column(perform_data$STA302.hours..W3.),
  week4_sta302 = num_NAs_in_column(perform_data$STA302.hours..W4.)
)))
```

```{R}
print(as_tibble(data.frame(
  quiz1_score = num_NAs_in_column(perform_data$Quiz_1_score),
  quiz2_score = num_NAs_in_column(perform_data$Quiz_2_score),
  quiz3_score = num_NAs_in_column(perform_data$Quiz_3_score),
  quiz4_score = num_NAs_in_column(perform_data$Quiz_4_score)
)))
```
...
```{r}
rows_with_no_country = cleaned_sta302_performance_data %>%
  select(-country)
```
...
# ggpairs(data = cleaned_sta302_performance_data)
```

## Slightly Zoomed In Pairwise Scatterplots

```{R}
# 1. Zoom in a bit by creating 3 - 4 pairs() functions:

# quiz4 ~ quiz 1, 2, 3
# covid4 ~ covid 1, 2, 3
# sta302h14 ~ sta302h1 1, 2, 3

```

## Top 4 - 5 Intersting Scatterplots

```{R}
# 2. Pick out 4 - 5 scatterplots that have interesting relationships


# 3. back up your choices with their correlation (R value).
